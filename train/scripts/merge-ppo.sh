# Merge PPO (GRPO) checkpoints to a single HuggingFace model.
#
...
#
# 2. Optional: save merged model to a target local dir (TARGET_DIR). Step auto-detected from latest checkpoint.
#    TARGET_DIR=train/my_merged bash train/scripts/merge-ppo.sh
#    Or set step explicitly: GLOBAL_STEP=30 TARGET_DIR=train/my_merged bash train/scripts/merge-ppo.sh
#
